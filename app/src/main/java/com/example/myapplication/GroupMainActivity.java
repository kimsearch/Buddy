...
public class GroupMainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rankingList.addAll(Arrays.asList(
                new RankingItem("준호", 95, R.drawable.profile1),
                new RankingItem("지수", 80, R.drawable.profile2),
                new RankingItem("민재", 67, R.drawable.profile3),
                new RankingItem("태연", 42, R.drawable.profile4)
        ));
    ...
    private void updateBarChart() {
        ...
        int index = 0;
        int size = dailyProgressMap.size();
        List<Map.Entry<String, Float>> allEntries = new ArrayList<>(dailyProgressMap.entrySet());

        for (int i = 0; i < allEntries.size(); i++) {
            BarEntry entry = new BarEntry(index++, allEntries.get(i).getValue());
            entries.add(entry);
            labels.add(allEntries.get(i).getKey());
        }
